...
export default async function MuseumPage() {
  ...
  return <main>
    ...
    <section className="collections" id="collections"><div className="section-heading"><div><p className="eyebrow">Explore the collection</p><h2>Every argument has an exhibit</h2></div></div><div className="collection-grid">
      {[["01", "Season archive", "Standings, playoff runs, and every year in context.", "/museum/seasons"], ["02", "Playoff museum", "Every bracket, bye, elimination, and championship path.", "/museum/playoffs"], ["03", "Rivalry room", "Career head-to-head records and bragging rights.", "/museum/rivalries"], ["04", "Draft vault", "Actual selections, values, and regrettable investments.", "/museum/drafts"], ["05", "Book of records", "High scores, low scores, and statistical wreckage.", "/museum/records"], ["06", "Front office ledger", "Waivers, free agents, drops, and preserved moves.", "/museum/transactions"], ["07", "JBL Historian", "Ask a grounded question about the official archive.", "/museum/historian"]].map(([number, title, description, href]) => <Link href={href} key={number}><article><span>{number}</span><h3>{title}</h3><p>{description}</p><small>Enter exhibit →</small></article></Link>)}
    </div></section>
