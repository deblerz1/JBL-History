...
export function ManagerScoreExhibit({corpus}:{corpus:HistorianCorpus}){
  ...
  return <section style={{marginTop:"3rem"}} aria-labelledby="manager-score-title">
    <h2 id="manager-score-title">The balanced ranking</h2>
    <p>{managerScoreVersion} · Published league formulas, scored out of 100.</p>
    {(["best_manager","worst_manager"] as const).map(intent=>{
      ...
      return <details key={intent} open={intent==="best_manager"} style={{marginTop:"1.5rem"}}>
        <summary>{intent==="best_manager"?"Best-manager score":"Worst-manager score"} — components and records</summary>
        <p>{response.answer}</p>
