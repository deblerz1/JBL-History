...
export async function interpretHistorianQuestion(question:string,corpus:HistorianCorpus,context?:HistorianContext):Promise<HistorianPlan|null>{
  ...
  const body={model:process.env.OPENAI_MODEL||"gpt-5.6-luna",store:false,input:[
    ...
    {role:"system",content:[{type:"input_text",text:`Metric catalog (shared with the deterministic calculator): ${metricPlannerCatalog()}. Compose any supported metric with phase, dates, member selection, ranking direction, rolling windows, minimum games and list output. No per-question template is required. Points against is total_points_against; an explicit per-game request is points_against_per_game. PA/PF is points_against_to_points_for; PF/PA is points_for_to_points_against. Ratios divide aggregate totals, not individual-game ratios. A follow-up asking for the inverse swaps those two ratio metrics while preserving filters. Scoring ratios are scoring balance, not proof of luck. Bare luckiest/unluckiest, holistic best/worst manager, and player-position/lineup metrics remain unsupported until their definitions and coverage are available; never silently replace them with wins or a ratio. Explicitly requested scoring ratios remain supported even when the user calls them luck. 'Best in playoffs' defaults to rank_metric, win_percentage, playoffs, highest; 'worst in playoffs' uses lowest. Explicit points, points against or points per game override this default. Playoff games mean completed championship-bracket matchups, possibly spanning multiple weeks, not individual scoring weeks. Do not invent a minimum sample; return the user's minimum or null. In a follow-up preserve windowYears unless changed; a standalone question without a stretch uses null.`}]},
